# Strands Agents on Raspberry Pi
...
hardware. They walk through the core building blocks (agent plus model, agent
plus tools, single agent over real device telemetry, multi-agent
orchestration) on a device small enough to sit on a desk, so you can see
end-to-end how an on-device agent observes its host and coordinates with
peers.
...
> **Demo code, not production.** These scripts are intended for learning and
> local experimentation on a personal Raspberry Pi. Review the
...
`/proc` directly and shell out to `vcgencmd`, so they only return meaningful
...
sanity check that Strands and Bedrock credentials are wired up.
...
to an explicit `BedrockModel` so you can see where temperature, region, and
other model settings get configured.
...
| `cpu_temperature`   | Current SoC temperature in Celsius (via `vcgencmd measure_temp`).  |
| `cpu_usage`         | CPU percent busy over a 0.5 s sample plus 1/5/15-minute load averages. |
| `memory_usage`      | Total, used, and available RAM in MB, plus used percent.           |
| `disk_usage`        | Total, used, and free GB for the filesystem containing a given path. |
| `uptime`            | Seconds since boot plus a pretty `Xd Yh Zm` form.                  |
| `throttling_status` | Decoded `vcgencmd get_throttled` flags (under-voltage, capping, and so on). |
...
| `performance_specialist`  | `cpu_usage`, `memory_usage`                                        | Live CPU and memory pressure.      |
| `capacity_specialist`     | `disk_usage`, `uptime`                                             | Storage and time since boot.       |

The orchestrator itself has **no telemetry tools**. Its only "tools" are the
three specialists, each exposed via `agent.as_tool(...)`. The orchestrator
routes the user's question to the smallest set of specialists that can
answer it, and calls all three in parallel for broad health checks.
...
- `cpu_temperature`: SoC temperature in Celsius.
- `cpu_usage`: CPU percent busy plus 1/5/15-minute load averages.
- `memory_usage`: total, used, and available RAM.
...
- All telemetry tools are **read-only**. They read from `/proc`,
  `/etc/os-release`, and `/proc/device-tree/model`, or shell out to
  `vcgencmd` for thermals and throttling state. Nothing in `pi_tools.py`
  writes to disk, modifies system state, or makes network calls.
- `subprocess` is invoked with **list arguments and no `shell=True`**, so
  the internal calls aren't exposed to shell injection.
...
- **Prompt-injection surface.** A chat user (or any text the model reads)
  can try to steer which tools get called. Impact here is bounded because
  every tool is read-only telemetry; there is no shell, file write, or
  arbitrary command tool. If you extend `pi_tools.py` with anything
  destructive (rebooting, killing processes, editing files, calling APIs),
  you must add your own confirmation or allow-list logic. Don't blindly
  hand the agent a shell tool.
...
- **No rate limiting or usage controls.** The chat loop invokes Bedrock on
  ...
  Monitor your account's usage and set Bedrock quotas and AWS Budgets
  alerts if you plan to leave it running.

**If you adapt this for anything beyond a personal demo**

- Put the chat behind auth (even basic SSH-only access counts).
- Move from a list of tools to an explicit allow-list per role, and never
  give an agent a tool more powerful than the user invoking it is allowed
  to run.
